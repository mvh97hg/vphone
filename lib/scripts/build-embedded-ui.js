...
async function buildEmbeddedHtml() {
    ...
    // Concatenate Phone/src files in order (same as bundle.js) to preserve global scope.
    // esbuild would wrap everything in an IIFE and rename variables, breaking cross-module
    // global references (e.g. localDB, HidePopup, getPicture used across files).
    const srcDir = path.join(phoneDir, 'src');
    const srcFiles = fs.readdirSync(srcDir)
        .filter(f => f.endsWith('.js') && f !== 'index.js')
        .sort();
    let phoneJsRaw = '';
    srcFiles.forEach((file) => {
        phoneJsRaw += '// File: ' + file + '\n';
        phoneJsRaw += readText(path.join(srcDir, file)) + '\n\n';
    });

    let phoneJsMin = await minifyJavaScript(phoneJsRaw, 'phone.js');
    phoneJsMin = phoneJsMin.replace(/\s*console\.log\("Audio:",[^\r\n]*\)\s*;?/g, '');
